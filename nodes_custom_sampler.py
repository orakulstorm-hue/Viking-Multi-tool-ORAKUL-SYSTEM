...
# Наша магия выравнивания (теперь работает с родными числами Флакса)
def oracle_align_sigmas(sigmas, total_steps):
    if len(sigmas) < 3:
        return sigmas
    
    s_np = sigmas.numpy()
    # Сохраняем 1.0 и 0.0, чтобы Флакс не сошел с ума
    start, end = s_np[0], s_np[-1]
    mid_values = s_np[1:-1]
    
    xs = np.linspace(0, 1, len(mid_values))
    ys = np.log(np.maximum(mid_values[::-1], 1e-10))
    
    # Распределяем промежуточные шаги логарифмически
    new_xs = np.linspace(0, 1, total_steps - 1)
    new_ys = np.interp(new_xs, xs, ys)
    
    new_mid = np.exp(new_ys)[::-1].copy()
    
    # Склеиваем обратно в идеальную кривую
    result = np.concatenate([[start], new_mid, [end]])
    ...
    def define_schema(cls):
        ...
        return io.Schema(
            ...
            inputs=[
                ...
                io.Combo.Input("scheduler", options=options),
